...
def generate_instrumentation_metadata(instrumentation_dir):
    ...
    llm = ChatOpenAI(model="gpt-4o-mini", temperature=0)  # Use cheaper model with higher limits
...
def save_yaml(generated_yaml, instrumentation_dir):
    # Clean the YAML before saving
    clean_yaml = generated_yaml.replace("```yaml", "").replace("```", "").strip()
    ...
    output_path = os.path.join(instrumentation_dir, "metadata.yaml")
    if os.path.exists(output_path):
        counter = 0
        # Find the next available filename like metadata.0.yaml, metadata.1.yaml, etc.
        while os.path.exists(os.path.join(instrumentation_dir, f"metadata.{counter}.yaml")):
            counter += 1
        output_path = os.path.join(instrumentation_dir, f"metadata.{counter}.yaml")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate metadata.yaml for an OpenTelemetry Python instrumentation.")
    parser.add_argument("instrumentation_dir", help="Path to an instrumentation directory.")
    args = parser.parse_args()
    yaml = generate_instrumentation_metadata(args.instrumentation_dir)
    save_yaml(yaml, args.instrumentation_dir)